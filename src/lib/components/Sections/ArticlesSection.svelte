<script>
  ...
  const isPreview = $derived(page.url.pathname !== '/stati/' && page.url.pathname !== '/stati');
...
<section class="py-8 w-full">
  ...
  <CardList items={displayItems} type="articles" />
  
  {#if isPreview && (section.items || []).length > 3}
    <div class="mt-10 text-center">
      <a href="/stati/" class="btn btn-outline btn-primary">
        Перейти к разделу статей
      </a>
    </div>
